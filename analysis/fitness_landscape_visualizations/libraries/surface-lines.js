...
AFRAME.registerComponent("dataline", {
    ...
    update: function(oldata){
        ...
        //  geometry = new THREE.Geometry();
         ...
         for (var i = 0; i < this.data.path.length; i++) {
             ...
             // console.log(vert);
             this.geometry.vertices[curr] = new THREE.Vector3( x_scale(vert[0]), z_scale(vert[2]), y_scale(vert[1]) );
             curr++;
         ...
         for (var i = 0.0; i < this.geometry.vertices.length; i+=2) {
            hsl_color = d3.hsl(0.0, 0.0, i/this.geometry.vertices.length);
            this.geometry.colors[i] = new THREE.Color(hsl_color.toString());
            this.geometry.colors[i+1] = new THREE.Color(hsl_color.toString());    
        }
    ...
    remove: function () {
        // Remove Object3D.
        this.el.removeObject3D('dataline');
      }
 });
