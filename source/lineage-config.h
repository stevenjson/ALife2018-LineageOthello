...
EMP_BUILD_CONFIG( LineageConfig,
  ...
  VALUE(EVAL_TIME, size_t, 256, "Agent evaluation time (how much time an agent has on a single turn)"),
  ...
  VALUE(RESOURCE_SELECT__RES_AMOUNT, double, 100.0, "Initial resource amount (for all resources)"),
  VALUE(RESOURCE_SELECT__RES_INFLOW, double, 100.0, "Resource in-flow (amount)"),
  ...
  VALUE(SCORE_MOVE__ILLEGAL_MOVE_VALUE, double, -1.0, "Score for making illegal move"),
  ...
  VALUE(AGP_GENOME_SIZE, size_t, 512, "How long should genome be?"),
  GROUP(SGP_PROGRAM_GROUP, "SignalGP program Settings"),
  VALUE(SGP_FUNCTION_LEN, size_t, 64, "Used for generating SGP programs. How long are functions?"),
  VALUE(SGP_FUNCTION_CNT, size_t, 8, "Used for generating SGP programs. How many functions do we generate?"),
  ...
  VALUE(SGP_PER_BIT__TAG_BFLIP_RATE, double, 0.05, "Per-bit mutation rate of tag bit flips."),
